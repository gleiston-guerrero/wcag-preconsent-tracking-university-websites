# -*- coding: utf-8 -*-
"""
sensitivity_inclusion.py - Sensibilidad a la regla de inclusion de sitios en la
replica multipunto.

El analisis principal (verify_multivantage.py) exige que un sitio se haya medido
...
Taxonomia: la lista extendida, identica a la de verify_multivantage.py.

ENTRADA:  los archivos results_<PUNTO>_r<PASADA>.json de la carpeta indicada.
SALIDA:   por consola, y opcionalmente en CSV con --csv.

Uso:
    python sensitivity_inclusion.py ../../data/raw/tracking
    python sensitivity_inclusion.py ../../data/raw/tracking --csv sensibilidad.csv
"""
...
# es r4. Identico a PAS en verify_multivantage.py.
# --------------------------------------------------------------------------
...
# verify_multivantage.py para que ambos scripts clasifiquen igual.
# --------------------------------------------------------------------------
...
def cargar(carpeta: Path):
    ...
    for punto, pasadas in PAS.items():
        for r in pasadas:
            ruta = carpeta / f"results_{punto}_r{r}.json"
            if not ruta.exists():
