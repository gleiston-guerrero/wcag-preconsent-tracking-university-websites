...
`figures_uais.py` y `fig_vantage.py` escriben sus PDF en el directorio actual. Las copias usadas en el artículo son las depositadas en `figures/`, y regenerarlas con los mismos datos reproduce esos ficheros byte a byte: los scripts suprimen la fecha de creación que matplotlib escribiría dentro de cada PDF, que era lo que hacía que la salida cambiara de una ejecución a otra. Si las regenera, copie las seis sobre `figures/` y `CHECKSUMS.txt` seguirá verificando. Se comprobó en dos sistemas —Linux con Python 3.11.15 y Windows con Python 3.10.3, ambos con matplotlib 3.10.9—, que producen ficheros idénticos. Las figuras necesitan Python 3.10 o posterior, porque matplotlib 3.10.9 no se instala en 3.9; los scripts estadísticos corren también en 3.9.13.
