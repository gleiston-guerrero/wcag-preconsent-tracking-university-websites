...
*English version: [SECOND_EVALUATOR_MANUAL.md](SECOND_EVALUATOR_MANUAL.md)*
...
portadas de quince universidades.

Independiente quiere decir: **no consulte codificaciones anteriores, ni resultados
...
- Si termina en **`_MODALABIERTO.csv`**, el aviso de cookies seguía abierto.
  Ciérrelo bien y vuelva a ejecutarlo. Borre el fichero incorrecto.
...
porque cada CSV lleva esos mismos datos dentro, en las columnas `dialog_state`,
`cmp_containers`, `cmp_excluded_elements` y `script_version`.
...
resultados esperados, hasta que haya entregado su codificación completa.
