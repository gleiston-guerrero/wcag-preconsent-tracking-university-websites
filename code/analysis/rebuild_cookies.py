#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reconstruye la tabla de cookies previas al consentimiento desde results.json.

Motivo: cookies_126_sites_v2.csv tomo la medicion de fuentes intermedias
...
expresiones regulares de audit.js, aplica la taxonomia extendida de forma
...
No inventa valores. Toda columna de medicion sale de results.json.

Uso:
    python rebuild_cookies.py <dir_datos> <dir_salida>
"""
import csv
import json
import re
...
# --- Taxonomia base: transcripcion literal de RASTREO en audit.js --------
BASE = [
...
def main(dir_datos, dir_salida):
    ...
    raw = json.loads((dd / "results.json").read_text(encoding="utf-8"))
    with (ds / "cookies_126_sites_v2.csv").open(encoding="utf-8-sig") as f:
    ...
    with (ds / "cookies_126_sites_v3.csv").open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=cab)
        w.writeheader()
        w.writerows(filas)

    with (ds / "cookies_unclassified_names.csv").open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["nombre_cookie", "sitios_en_que_aparece"])
        for n, k in sorted(sin_clasificar.items(), key=lambda x: (-x[1], x[0])):
            w.writerow([n, k])

    with (ds / "cookies_divergences_v2.csv").open("w", encoding="utf-8", newline="") as f:
